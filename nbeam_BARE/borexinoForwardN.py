...
#EDIT THIS W/ FINAL MEASUREMENTS OF P_Z
measPzs = [-0.22+0.18,-0.26+0.16,-0.30+0.18,0,0,0,0,0] #measured Pz's at the endpoint
...
class network:
    def __init__(self):
        ...
        #EDIT THIS WITH BEAM ENERGIES
        self.energies = [self.Escale*7.4/hbarc,self.Escale*8.1/hbarc, 
                         self.Escale*9.7/hbarc,self.Escale*11.2/hbarc,
                         self.Escale*14.0/hbarc,self.Escale*17.2/hbarc,
                         self.Escale*18.6/hbarc,self.Escale*20.1/hbarc] #array of energies - numbers in MeV
        self.Nbeams = len(self.energies)

        '''Define initial conditions on flavor:'''
        #initial conditions - using all Pz's at 1 and everything else at 0 for pure e- flavor
        self.init = []
        for n in range(self.Nbeams):
            self.init.append(0)
            self.init.append(0)
            self.init.append(1)
        #self.Pz_1_init = 1. 
        ...
        for i in range(self.Nbeams):
            print('E'+str(i)+':', self.energies[i]) #print energies
        #print('E1:', self.E1)
        #print('E2:', self.E2)
        #print('E3:', self.E3)
        ...
        for E in self.energies:
            self.delta_vacs.append(self.dmsq/(2*E))
...
#        self.injdt = self.dt # one-to-one injection:step
    ...
    def eqns(self,X, t): 
        P=X[:-1] #P is first N*3 elements
        r=X[-1] #r is last element
        #P1x, P1y, P1z, P2x, P2y, P2z, P3x, P3y, P3z, r = X #!

        ### NEUTRINO 1 at Energy 1 ###
        print("here")
        for dvac in self.delta_vacs:
            print(dvac)
        ...
        dPdt = []
        for n in range(self.Nbeams): #each beam
            dPdt.append((self.delta_vacs[n]*np.cos(2.0*self.theta)-self.V_matter(t))*P[n*3+1])#x
            dPdt.append((-self.delta_vacs[n]*np.cos(2.0*self.theta) + self.V_matter(t))*P[n*3]
                      - self.delta_vacs[n]*np.sin(2.0*self.theta)*P[n*3+2])
            dPdt.append(self.delta_vacs[n]*np.sin(2.0*self.theta)*P[n*3+1])
        ...
        return tuple(dPdt) #don't think it needs to be a tuple but it was before so leaving it as one for now
    ...
    def DoIt(self):
        ...
        for n in range(self.Nbeams):
            for i in range(3):
                beamtype = ('x','y','z')[i]
                sp.savetxt('%s/P%s%s_%s.txt' %(self.path, n+1, beamtype, self.label),self.sim[:,n*3+i])
        ...
        for n in range(self.Nbeams):
            Paltered = self.sim[:,n*3+2]
            Paltered[-1]=measPzs[n]
            sp.savetxt('%s/P%szA_%s.txt' %(self.path, n+1, self.label),Paltered)
...
#        np.savetxt('%s/init_trueSolution_Pz1IC%0.2f_Pz2IC%0.2f_N%i.dat' % (self.path, self.Pz_1_init, self.Pz_2_init, self.Nlocations),M)    
        ...
        for i in range(self.Nbeams):
            fig=plt.figure(i)
            plt.title('Neutrino %s' % (i+1))
            plt.subplot(311)
            plt.ylim((ymin,ymax))
            plt.ylabel('Px,%s' % (i+1), fontsize=12)
            plt.plot(times,sim[:,i*3+0],'g')#,linestyle='dotted') 
            plt.subplot(312)
            plt.ylim((ymin,ymax))
            plt.ylabel('Py,%s' % (i+1), fontsize=12)
            plt.plot(times,sim[:,i*3+1],'g')#,linestyle='dotted')
            plt.subplot(313)
            plt.ylim((ymin,ymax))
            plt.ylabel('Pz,%s' % (i+1), fontsize=12)
            plt.plot(times,sim[:,i*3+2],'g') 
...
#         fig1.savefig('%s/P4P5P6_%s.png' % (self.path, self.label))         
        ...
        fig2 = plt.figure(self.Nbeams)
        plt.title('Radius')
        plt.subplot(111)
        plt.ylabel('radius', fontsize=12)
        plt.plot(times,sim[:,self.Nbeams*3],'b') 
#        plt.legend()
        plt.show()
        fig2.savefig('%s/radius_%s.png' % (self.path, self.label))    

        
        fig3 = plt.figure(self.Nbeams+1)
        plt.title('n_e')
        plt.subplot(111)
        plt.ylabel('n_e', fontsize=12)
        plt.yscale("log")
        plt.plot(times,self.Vmarr,'b') 
        for delta_vac in self.delta_vacs:
            plt.plot(times,np.ones_like(times)*delta_vac*np.cos(2.0*self.theta))
#        plt.legend()
        plt.show()
